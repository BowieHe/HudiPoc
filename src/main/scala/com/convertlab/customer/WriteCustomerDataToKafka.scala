...
object WriteCustomerDataToKafka {
  def main(args: Array[String]): Unit = {
    println("==== start processing")
    val spark = SparkSession
      .builder.enableHiveSupport()
      .appName("WriteCustomerDataToKafka")
      .getOrCreate()
    val sc = spark.sparkContext
    sc.setLogLevel("ERROR")

    val originDs: DataFrame = spark.sql("select * from customer_ro")
    originDs.createOrReplaceTempView("customer_temp_view")
    for (i <- 1 to 2) {
      println(LocalDateTime.now() + s"===== start write data into kafka, index $i")
      val beginIndex = i * 100000
      val endIndex = (i + 1) * 100000
      val updateDs = spark.sql(s"select * from customer_temp_view where _id > $beginIndex and _id < $endIndex")
      ...
      updateDs
        ...
        .option("topic", "kafka_hudi_customer")
        .save()
